// Code generated by protoc-gen-micro. DO NOT EDIT.
// source: proto/PutUserInfo/PutUserInfo.proto

package go_micro_srv_PutUserInfo

import (
	fmt "fmt"
	proto "github.com/golang/protobuf/proto"
	math "math"
)

import (
	context "context"
	client "github.com/micro/go-micro/client"
	server "github.com/micro/go-micro/server"
)

// Reference imports to suppress errors if they are not otherwise used.
var _ = proto.Marshal
var _ = fmt.Errorf
var _ = math.Inf

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = proto.ProtoPackageIsVersion3 // please upgrade the proto package

// Reference imports to suppress errors if they are not otherwise used.
var _ context.Context
var _ client.Option
var _ server.Option

// Client API for PutUserInfo service

type PutUserInfoService interface {
	CallPutUserInfo(ctx context.Context, in *Request, opts ...client.CallOption) (*Response, error)
}

type putUserInfoService struct {
	c    client.Client
	name string
}

func NewPutUserInfoService(name string, c client.Client) PutUserInfoService {
	if c == nil {
		c = client.NewClient()
	}
	if len(name) == 0 {
		name = "go.micro.srv.PutUserInfo"
	}
	return &putUserInfoService{
		c:    c,
		name: name,
	}
}

func (c *putUserInfoService) CallPutUserInfo(ctx context.Context, in *Request, opts ...client.CallOption) (*Response, error) {
	req := c.c.NewRequest(c.name, "PutUserInfo.CallPutUserInfo", in)
	out := new(Response)
	err := c.c.Call(ctx, req, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

// Server API for PutUserInfo service

type PutUserInfoHandler interface {
	CallPutUserInfo(context.Context, *Request, *Response) error
}

func RegisterPutUserInfoHandler(s server.Server, hdlr PutUserInfoHandler, opts ...server.HandlerOption) error {
	type putUserInfo interface {
		CallPutUserInfo(ctx context.Context, in *Request, out *Response) error
	}
	type PutUserInfo struct {
		putUserInfo
	}
	h := &putUserInfoHandler{hdlr}
	return s.Handle(s.NewHandler(&PutUserInfo{h}, opts...))
}

type putUserInfoHandler struct {
	PutUserInfoHandler
}

func (h *putUserInfoHandler) CallPutUserInfo(ctx context.Context, in *Request, out *Response) error {
	return h.PutUserInfoHandler.CallPutUserInfo(ctx, in, out)
}
