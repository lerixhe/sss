...
type User struct {
	...
	Real_name     string        `orm:"size(32)" json:"real_name"`     //真实姓名 实名认	证
	Id_card       string        `orm:"size(20)" json:"id_card"`       //身份证号 实名认	证
	Avatar_url    string        `orm:"size(256)" json:"avatar_url"`   //用户头像路径	通过fastdfs进行图片存储
	Houses        []*House      `orm:"reverse(many)" json:"houses"`   //用户发布的房屋信	息 一个人多套房
...
type House struct {
	...
	//预定完成的该房屋的订单数
	Index_image_url string `orm:"size(256)" orm:"default("")"
...
func (this *House) To_one_house_desc() interface{} {
	...
	if err != nil {
		beego.Error("select orders comments error, err =", err, "house id = ",
			this.Id)
	}
...
func init() {
	...
	orm.RegisterDataBase(
		"default", "mysql",
		utils.G_mysql_dbname+":123456@tcp("+utils.G_mysql_addr+":"+utils.G_mysql_port+")/go1micro?charset=utf8", 30)
	//注册model
